...
. Select the consistency group member volume that you want to edit, and then click the menu:ellipsis icon[Edit Settings].
+
The Edit consistency group member volume window appears.
...
a|
Use the bar to adjust the percentage point at which System Manager sends an alert notification when the reserved capacity for a member volume is nearing full.
