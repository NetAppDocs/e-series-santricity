...
. Click the menu:ellipsis icon[Change volume cache settings].
+
The Change volume cache Settings window appears. All volumes on the storage array appear in this dialog box.

. Check the box next to each volume for which you want to change the cache settings or check the box in the table header to select all volumes.
+
