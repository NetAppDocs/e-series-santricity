...
+
To manually create a pool: Select *Storage*, and then select *Pools and volume groups*. From the All Capacity tab, click menu:Create[Pool].
...
+
To manually add drives to an existing pool: From the Pools and volume groups page, select the pool, and then click *Add capacity*.
...
You can change the preservation capacity from *Pools and volume groups*. Select the pool that you want to edit. Click the menu:ellipsis icon[Edit Settings], and then select the *Settings* tab.
 ...
 ** You must have an even number of drives in the volume group. If you do not have an even number of drives and you have some remaining unassigned drives, go to *Pools and volume groups* to add additional drives to the volume group, and retry the operation.
...
To view SSD Cache statistics, select menu:Storage[Pools and volume groups] from the left panel. Select the SSD Cache that you want to view statistics for, and then click the menu:ellipsis icon[View statistics]. The nominal statistics are shown on the View Statistics page.
...
[NOTE]
====
The Additional Optimization Capacity slider is only available for EF80, EF50, EF600, and EF300 storage systems.
====
...
Resource Provisioning is a feature available in the EF80, EF50,  EF300, and EF600 storage arrays, which allows volumes to be put in use immediately with no background initialization process.
...
Resource provisioning is enabled by default on systems where the drives support DULBE. You can disable that default setting from *Pools and volume groups*.
