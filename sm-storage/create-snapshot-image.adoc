...
. Do one of the following actions to create a snapshot image:
  ** From the left panel, select menu:Storage[Volumes]. Select the object (base volume or snapshot consistency group), and then click the menu:ellipsis icon[Create instant snapshot].
  ** From the left panel, select menu:Storage[Snapshots]. Select the *Snapshot Images* tab, and then select menu:Create[Instant snapshot Image].
+
