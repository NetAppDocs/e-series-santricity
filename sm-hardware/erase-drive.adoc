...
. From the context menu, select *Erase Drives*.
+
...
. Confirm the operation by typing `erase`, and then click *Erase Drives*.
+
[CAUTION]
====
...
The Erase operation might take several minutes or several hours. You can view the status in menu:Storage[Operations in Progress]. When the Erase operation completes, the drives are available for use in another volume group or disk pool, or in another storage array.
